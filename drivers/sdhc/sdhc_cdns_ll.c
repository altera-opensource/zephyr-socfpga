...
static struct sdhc_cdns_combo_phy sdhc_cdns_combo_phy_reg;
static struct sdhc_cdns_sdmmc sdhc_cdns_sdmmc_reg;
...
static int sdhc_cdns_init(void)
{
	...
	ret = sdhc_cdns_program_phy_reg(&sdhc_cdns_combo_phy_reg, &sdhc_cdns_sdmmc_reg);
	if (ret != 0U) {
		LOG_ERR("SoftPhy register configuration failed");
		return ret;
	}

	ret = sdhc_cdns_init_hrs_io(&sdhc_cdns_combo_phy_reg, &sdhc_cdns_sdmmc_reg);
	if (ret != 0U) {
	...
	const struct sdhc_cdns_ops  **cb_sdmmc_ops)
{
	...
	*cb_sdmmc_ops = &cdns_sdmmc_ops;
	...
	cdns_sdhc_set_sdmmc_params(&sdhc_cdns_combo_phy_reg, &sdhc_cdns_sdmmc_reg);
}
