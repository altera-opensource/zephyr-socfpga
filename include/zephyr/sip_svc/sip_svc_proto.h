...
#ifndef ZEPHYR_INCLUDE_SERVICE_SIP_SVC_PROTO_H_
#define ZEPHYR_INCLUDE_SERVICE_SIP_SVC_PROTO_H_
...
#define SIP_SVC_PROTO_HEADER(code, trans_id) \
	(((code) << SIP_SVC_PROTO_HEADER_CODE_OFFSET) & \
		SIP_SVC_PROTO_HEADER_CODE_MASK) | \
	(((trans_id) << SIP_SVC_PROTO_HEADER_TRANS_ID_OFFSET) & \
		SIP_SVC_PROTO_HEADER_TRANS_ID_MASK) | \
	((SIP_SVC_PROTO_VER << SIP_SVC_PROTO_HEADER_VER_OFFSET) & \
		SIP_SVC_PROTO_HEADER_VER_MASK)

#define SIP_SVC_PROTO_HEADER_GET_CODE(header) \
	((header) >> SIP_SVC_PROTO_HEADER_CODE_OFFSET & \
		SIP_SVC_PROTO_HEADER_CODE_MASK)

#define SIP_SVC_PROTO_HEADER_GET_TRANS_ID(header) \
	((header) >> SIP_SVC_PROTO_HEADER_TRANS_ID_OFFSET & \
		SIP_SVC_PROTO_HEADER_TRANS_ID_MASK)

#define SIP_SVC_PROTO_HEADER_SET_TRANS_ID(header, trans_id) \
	(header) &= ~(SIP_SVC_PROTO_HEADER_TRANS_ID_MASK << \
 ...
 *       SMC/HVC
 *     - a0 represent Cancel Transaction ID in Cancel command
 *
 * resp_data_addr
 ...
 *       This address is provided by client during request. Client
 *       responsible to free the memory space when receive the callback
 *       of a asynchronous command transaction.
 *
 * resp_data_size
 *     - This parameter only used by asynchronous command.
 ...
 *
 * priv_data
 *     - Memory address to client context which diven during request.
 */
...
#endif /* ZEPHYR_INCLUDE_SERVICE_SIP_SVC_PROTO_H_ */
